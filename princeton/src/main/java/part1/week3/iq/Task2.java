...
public class Task2 {
    ...
    private int inversionCount(int[] a, int[] aux, int low, int mid, int high) {
        int i = low;
        int j = mid + 1;
    ...
    private int inversionCount(int[] a, int[] aux, int low, int high) {
        if (low >= high) {
            return 0;
        }
        ...
        int inversionCount = inversionCount(aux, a, low, mid);
